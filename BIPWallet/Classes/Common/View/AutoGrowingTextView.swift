...
@objc protocol AutoGrowingTextViewDelegate: UITextViewDelegate {
	@objc optional func textViewDidChangeHeight(_ textView: AutoGrowingTextView, height: CGFloat)
}

@IBDesignable @objc
class AutoGrowingTextView: DefaultTextView {
	...
	override open func layoutSubviews() {
		...
		if height != heightConstraint!.constant {
			...
			if let delegate = delegate as? AutoGrowingTextViewDelegate {
...
//			self.scrollRangeToVisible(NSMakeRange(-1, 0)) // Scroll to bottom
	...
	@objc func textDidEndEditing(notification: Notification) {
		if let sender = notification.object as? AutoGrowingTextView, sender == self {
